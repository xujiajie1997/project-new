...
class MotorMoveCANBUS:
	def __init__(self):
		...
		while not rospy.is_shutdown():
			...
			self.send_group1(0x603,self.sendingdata)
			self.sendingdata=[0x2b,0x40,0x60,0x00,0x0f,0x00,0x00,0x00]
			self.send_group1(0x603,self.sendingdata) 
			self.sendingdata=[0x23,0xff,0x60,0x00,0xb8,0x0b,0x00,0x00]			
			self.send_group1(0x603,self.sendingdata)'''
			#self.sendingdata=[0x23,0xff,0x60,0x00,0xac,0x0d,0x00,0x00]			
			#self.send_group1(0x603,self.sendingdata)
			for i in range(0,8):
				self.sendingdata=[0x2b,0x40,0x60,0x00,0x07,0x00,0x00,0x00]
			...
			'''self.sendingdata=[0x2b,0x40,0x60,0x00,0x07,0x00,0x00,0x00]
			self.send_group(0x606,self.sendingdata) 
			self.sendingdata=[0x2b,0x40,0x60,0x00,0x07,0x00,0x00,0x00]
			self.send_group(0x608,self.sendingdata) '''
